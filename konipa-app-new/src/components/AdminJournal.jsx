...
import adminJournalService from '../services/adminJournalService';
...
const AdminJournal = () => {
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        setIsLoading(true);
        await adminJournalService.loadActivities();
        loadData();
        previousActivitiesCount.current = adminJournalService.getAllActivities().length;
      } catch (error) {
        } finally {
        setIsLoading(false);
      }
  ...
  const loadData = async (showLoading = false) => {
    ...
    try {
      ...
      switch (viewMode) {
        case 'daily':
          loadedActivities = adminJournalService.getActivitiesByDate(selectedDate);
          setDailySummary(adminJournalService.getDailySummary(selectedDate));
          break;
        case 'weekly':
          const weekStart = new Date(selectedDate);
          weekStart.setDate(weekStart.getDate() - weekStart.getDay());
          loadedActivities = adminJournalService.getAllActivities().filter(activity => {
          ...
          break;
        case 'all':
          loadedActivities = adminJournalService.getAllActivities();
          break;
        default:
          loadedActivities = adminJournalService.getActivitiesByDate(selectedDate);
      }
      ...
      setStatistics(adminJournalService.getStatistics());
  ...
  const exportData = () => {
    const csv = adminJournalService.exportToCSV(selectedDate, selectedDate);
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `journal-admin-${selectedDate}.csv`;
    a.click();
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Journal Admin</h1>
